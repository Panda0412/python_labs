def encrypt_vigenere(plaintext, keyword):
    ...
    for i in range(len(plaintext)):
        ...
        if ord('A') <= simkey <= ord('Z'):
            ...
            simkey -= ord('a')
        if ord('A') <= simbol <= ord('Z')-simkey or \
                ord('a') <= simbol <= ord('z')-simkey:
            ciphertext += chr(simbol+simkey)
        elif ord('Z')-simkey < simbol <= ord('Z') or \
                ord('z')-simkey < simbol <= ord('z'):
...
def decrypt_vigenere(ciphertext, keyword):
    ...
    for i in range(len(ciphertext)):
        ...
        if ord('A') <= simkey <= ord('Z'):
            ...
            simkey -= ord('a')
        if ord('A')+simkey <= simbol <= ord('Z') or \
                ord('a')+simkey <= simbol <= ord('z'):
            plaintext += chr(simbol-simkey)
        elif ord('A') <= simbol < ord('A')+simkey or \
                ord('a') <= simbol < ord('a')+simkey:
