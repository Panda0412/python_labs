        ...
        for rown, row in enumerate(celllist.clist):
            for coln, col in enumerate(row):
                if col.is_alive():
                    ...
                    pygame.draw.rect(self.screen, pygame.Color('light blue'),
                                     (rown * self.cell_size,
                                      coln * self.cell_size,
                                      self.cell_size, self.cell_size))
...
class CellList:
    ...
    def __init__(self, nrows, ncols, randomize=False):
        ...
        if randomize:
            clist = [[Cell(rown, coln, state=bool(random.randint(0, 1)))
                      for coln in range(ncols)] for rown in range(nrows)]
        else:
            clist = [[Cell(rown, coln, state=False)
                      for coln in range(ncols)] for rown in range(nrows)]
        self.clist = clist
        ...
        for cell in self:
            ...
            for neighbour in neighbours:
                if neighbour.is_alive():
                    neighbours_num += 1
            if (neighbours_num == 2 and cell.is_alive()) \
                    or neighbours_num == 3:
                new_clist[cell.row][cell.col] = \
                    Cell(cell.row, cell.col, state=True)
            else:
                new_clist[cell.row][cell.col] = \
                    Cell(cell.row, cell.col, state=False)
